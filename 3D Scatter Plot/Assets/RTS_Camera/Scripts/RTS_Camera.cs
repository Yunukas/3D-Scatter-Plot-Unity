...
namespace RTS_Cam
{
    ...
    public class RTS_Camera : MonoBehaviour
    {
        ...
        private void Move()
        {
            // if user attemps a camera movement, close the panel
            if (Input.GetKey(zoomInKey) || Input.GetKey(zoomOutKey) || Input.GetKey(panningKey)
                || Input.GetKey(rotateLeftKey) || Input.GetKey(rotateRightKey) || Input.GetKey(mouseRotationKey)
                || System.Math.Abs(Input.GetAxis(horizontalAxis)) > 0
                || System.Math.Abs(Input.GetAxis(verticalAxis)) > 0
                || System.Math.Abs(Input.GetAxis(zoomingAxis)) > 0)
            {
            ...
            if (useScreenEdgeInput)
            {
                ...
                if(leftRect.Contains(MouseInput) || rightRect.Contains(MouseInput)
                    || upRect.Contains(MouseInput) || downRect.Contains(MouseInput))
                {
                    PanelControl.RemovePanel();
                }
